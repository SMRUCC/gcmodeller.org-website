...
const NODES = [
  {
    ...
    tag: "Modeling",
    hub: true,
    desc: "A computational replica of living cellular physiology: reconstruct metabolism, regulation and signaling, then run experiments in silico before touching the bench.",
  },
  {
    name: "Synthetic Biology",
    tag: "Design",
    hub: true,
    desc: "Designing biological parts, devices and systems with engineering principles — computer-assisted design applied to life itself.",
  },
  {
    name: "Systems Biology",
    tag: "Modeling",
    hub: false,
    desc: "Treating the cell as an integrated network of interacting components, where emergent behavior cannot be explained gene by gene.",
  },
  {
    name: "Genomics",
    tag: "Data",
    hub: false,
    desc: "Sequencing the blueprint of life — assembly, annotation and comparative analysis of genomes at population and ecosystem scale.",
  },
  {
    name: "Gene Regulation",
    tag: "Modeling",
    hub: false,
    desc: "The wiring diagram of transcription factors and their targets: the logic that lets one genome produce hundreds of distinct cell states.",
  },
  {
    name: "Metabolic Modeling",
    tag: "Modeling",
    hub: false,
    desc: "Flux balance analysis converts network stoichiometry into testable predictions of growth rates and secretion profiles.",
  },
  {
    name: "Neural Networks",
    tag: "AI",
    hub: false,
    desc: "Graph and neural models — GNN, LNN, neural ODEs — learn the hidden logic of living systems directly from high-throughput measurements.",
  },
  {
    name: "Genome Annotation",
    tag: "Pipeline",
    hub: false,
    desc: "ORF prediction, homology search, HMM profiling and motif discovery turn raw sequence into functional knowledge.",
  },
  {
    name: "Multi-Omics",
    tag: "Data",
    hub: false,
    desc: "Integrating transcriptome, proteome, metabolome and microbiome layers into one coherent picture of a cell at work.",
  },
  {
    name: "Transcriptomics",
    tag: "Omics",
    hub: false,
    desc: "Measuring every mRNA the cell chooses to express — the first readout of both natural and virtual perturbations.",
  },
  {
    name: "Proteomics",
    tag: "Omics",
    hub: false,
    desc: "The working machinery of the cell: abundance, interaction and modification of the complete protein complement.",
  },
  {
    name: "Metagenomics",
    tag: "Omics",
    hub: false,
    desc: "Reading the collective genome of microbial communities that shape health, environment and fermentation.",
  },
  {
    name: "Metabolomics",
    tag: "Omics",
    hub: false,
    desc: "The layer closest to the phenotype — small molecules that report what the cell is actually doing right now.",
  },
  {
    name: "Perturbation Analysis",
    tag: "Simulation",
    hub: false,
    desc: "Knock out a gene or over-express a pathway in the virtual cell first — then validate only the most promising designs at the bench.",
  },
  {
    name: "Pathway Enrichment",
    tag: "Analysis",
    hub: false,
    desc: "GSEA, GSVA and over-representation analysis translate gene lists into biological meaning on KEGG and GO maps.",
  },
  {
    name: "R# Language",
    tag: "Interface",
    hub: true,
    desc: "A dynamic scripting language for scientific computing — the single interface that drives every GCModeller workflow.",
  },
];
...
function makeLabel(text, hub) {
  ...
  const font =
    (hub ? "500 " : "400 ") + fs + 'px Inter, "Segoe UI", sans-serif';
  ctx.font = font;
...
for (let i = 0; i < N; i++) {
  ...
  for (let j = 0; j < N; j++)
    if (j !== i) {
      dists.push([
        nodeVisuals[i].position.distanceTo(nodeVisuals[j].position),
        j,
      ]);
    }
...
spokeGeo.setAttribute(
  "position",
  new THREE.BufferAttribute(new Float32Array(hubIdx.length * 6), 3),
);
...
function makeRing() {
  ...
  const ctx = c.getContext("2d");
  ctx.strokeStyle = "rgba(255,96,80,0.95)";
  ctx.lineWidth = 5;
  ctx.beginPath();
  ctx.arc(64, 64, 52, 0, Math.PI * 2);
  ctx.stroke();
  ctx.strokeStyle = "rgba(255,96,80,0.3)";
  ctx.lineWidth = 2;
  ctx.beginPath();
...
function buildHighlight(i) {
  ...
  g.setAttribute(
    "position",
    new THREE.BufferAttribute(new Float32Array(mine.length * 6), 3),
  );
...
function select(i) {
  ...
  if (i >= 0) {
    const d = NODES[i];
    swapPanel(
      "KNOWLEDGE NETWORK · NODE " + pad2(i + 1) + " / " + N,
      d.name,
      '<span class="ndot">●</span> ' + d.tag.toUpperCase(),
      d.desc,
    );
...
window.__nodeScreen = (i) => {
  const v = nodeVisuals[i].position
    .clone()
    .applyMatrix4(netGroup.matrixWorld)
    .project(camera);
  return [
